...
export const SocialMediaStatus = ({ article }: SocialMediaStatusProps) => {
  return (
    <div className="flex items-center gap-2">
      ...
      {article.linkedin_post_url && (
        <Badge variant="secondary" className="flex items-center gap-1">
          ...
          <Button
            ...
            size="sm"
            className="h-4 w-4 p-0"
            onClick={() => window.open(article.linkedin_post_url, '_blank')}
          >
            <ExternalLink className="w-3 h-3" />
          </Button>
        </Badge>
      ...
      {(article.instagram_enabled || article.twitter_enabled || article.linkedin_enabled) && 
       !article.instagram_post_url && !article.twitter_post_url && !article.linkedin_post_url && (
